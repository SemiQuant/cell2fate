...
def download_ncbi_homologene_mapping(output_dir='gene_mapping'):
    """
    Download Mouse-to-Human gene mapping from NCBI HomoloGene.
    This uses the same ortholog data that many tools including Enrichr rely on.
    
    Parameters
    ----------
    output_dir
        Directory to save mapping files. Defaults to 'gene_mapping'.
    ...
    mapping_file = output_path / "ncbi_homologene_mapping.csv"
    
    if mapping_file.exists():
        print(f"NCBI HomoloGene mapping file already exists: {mapping_file}")
        return str(mapping_file)
    
    print("Downloading Mouse-to-Human gene mapping from NCBI HomoloGene...")
    
    try:
        # Download HomoloGene data
        url = "https://ftp.ncbi.nlm.nih.gov/pub/HomoloGene/current/homologene.data"
        
        response = requests.get(url, timeout=30)
        response.raise_for_status()
        
        # Parse HomoloGene data
        # Format: tax_id, gene_id, gene_symbol, protein_gi, protein_accession
        mapping_data = []
        ...
        for line in response.text.strip().split('\n'):
            ...
            if len(parts) >= 5:
                ...
                # Human (tax_id: 9606) and Mouse (tax_id: 10090)
                if tax_id in ['9606', '10090']:
                    mapping_data.append({
                        'tax_id': tax_id,
                        'gene_id': gene_id,
        ...
        # Group by gene_id to find orthologs
        ortholog_groups = df.groupby('gene_id')
        
        mouse_human_mapping = []
        for gene_id, group in ortholog_groups:
            if len(group) == 2:  # Both human and mouse present
                human_row = group[group['tax_id'] == '9606']
                mouse_row = group[group['tax_id'] == '10090']
                
                if len(human_row) > 0 and len(mouse_row) > 0:
                    mouse_human_mapping.append({
                        'mouse_symbol': mouse_row.iloc[0]['gene_symbol'],
                        'human_symbol': human_row.iloc[0]['gene_symbol'],
                        'gene_id': gene_id
                    })
        
        # Create final mapping DataFrame
        mapping_df = pd.DataFrame(mouse_human_mapping)
        mapping_df = mapping_df.drop_duplicates()
        mapping_df.to_csv(mapping_file, index=False)
        
        print(f"Downloaded {len(mapping_df)} Mouse-Human ortholog mappings to {mapping_file}")
        return str(mapping_file)
        
    except Exception as e:
        print(f"Error downloading NCBI HomoloGene mapping: {e}")
        print("Falling back to basic mapping...")
        
        # Fallback: create basic mapping
        basic_mapping = {
            'H2-Ab1': 'HLA-DQB1', 'Irf8': 'IRF8', 'Canx': 'CANX', 'Psme2': 'PSME2',
            'H2az1': 'H2AFZ', 'Txn1': 'TXN1', 'Cd63': 'CD63', 'Efhd2': 'EFHD2',
            'Gngt2': 'GNGT2', 'Ubb': 'UBB', 'Sell': 'SELL', 'H2-DMb1': 'HLA-DMB',
            'Rpl10': 'RPL10', 'Dusp1': 'DUSP1', 'Psme1': 'PSME1', 'Rps21': 'RPS21',
            'Calr': 'CALR', 'Bag1': 'BAG1', 'Anp32b': 'ANP32B', 'Dazap2': 'DAZAP2',
            'Cd52': 'CD52', 'Lyz2': 'LYZ', 'Rpl41': 'RPL41', 'Tmsb10': 'TMSB10',
            'Ifitm3': 'IFITM3', 'Rps20': 'RPS20', 'Cxcl10': 'CXCL10', 'Fth1': 'FTH1',
            'Rps27a': 'RPS27A', 'S100a6': 'S100A6'
        }
        
        df = pd.DataFrame([
            {'mouse_symbol': mouse, 'human_symbol': human, 'gene_id': ''}
            for mouse, human in basic_mapping.items()
        ])
        df.to_csv(mapping_file, index=False)
        
        print(f"Created basic mapping with {len(df)} genes to {mapping_file}")
        return str(mapping_file)

def download_gene_mapping(output_dir='gene_mapping'):
    """
    Download Mouse-to-Human gene name mapping using NCBI HomoloGene.
    This uses the same ortholog data that many tools including Enrichr rely on.
    ...
    str
        Path to the mapping file.
    """
    return download_ncbi_homologene_mapping(output_dir)

def load_gene_mapping(mapping_file):
    """
    Load gene mapping from file.
    ...
    -------
    dict
        Dictionary mapping mouse symbols to human symbols.
    """
    import pandas as pd
...
def convert_gene_list(gene_list, mapping_dict, direction='mouse_to_human'):
    """
    Convert gene list between Mouse and Human symbols.
    ...
    mapping_dict
        Dictionary mapping between species.
    direction
        'mouse_to_human' or 'human_to_mouse'.
    ...
    elif direction == 'human_to_mouse':
        reverse_mapping = {v: k for k, v in mapping_dict.items()}
        return [reverse_mapping.get(gene, gene) for gene in gene_list]
    else:
        raise ValueError("Direction must be 'mouse_to_human' or 'human_to_mouse'")
...
def offline_enrichment_with_mapping(gene_list, background, gene_sets_dict, p_adj_cutoff=0.01, mapping_file=None):
    """
    Perform offline enrichment analysis with Mouse-to-Human gene mapping.
    
    Parameters
    ----------
    gene_list
        List of Mouse genes to test for enrichment.
    background
        List of background Mouse genes.
    gene_sets_dict
        Dictionary mapping gene set names to lists of Human genes.
    p_adj_cutoff
        Adjusted p-value cutoff for significance.
    mapping_file
        Path to Mouse-to-Human gene mapping file. If None, will download/create one.
        
    Returns
    -------
    pandas.DataFrame
        DataFrame with enrichment results (genes converted back to Mouse symbols).
    """
    import pandas as pd
    from scipy.stats import hypergeom
    from statsmodels.stats.multitest import multipletests
    
    # Download/create gene mapping if not provided
    if mapping_file is None:
        mapping_file = download_gene_mapping()
    
    # Load gene mapping
    mouse_to_human = load_gene_mapping(mapping_file)
    human_to_mouse = {v: k for k, v in mouse_to_human.items()}
    
    # Convert Mouse genes to Human genes
    human_gene_list = convert_gene_list(gene_list, mouse_to_human, 'mouse_to_human')
    human_background = convert_gene_list(background, mouse_to_human, 'mouse_to_human')
    
    print(f"Converted {len(gene_list)} Mouse genes to {len(set(human_gene_list))} unique Human genes")
    print(f"Converted {len(background)} Mouse background genes to {len(set(human_background))} unique Human genes")
    
    results = []
    
    for gene_set_name, gene_set_genes in gene_sets_dict.items():
        # Convert to sets for efficient operations
        human_gene_list_set = set(human_gene_list)
        human_background_set = set(human_background)
        gene_set_set = set(gene_set_genes)
        
        # Find overlaps
        genes_in_set = human_gene_list_set.intersection(gene_set_set)
        genes_in_background = human_background_set.intersection(gene_set_set)
        ...
        N = len(human_background_set)  # Total genes in background
        K = len(genes_in_background)  # Genes in gene set that are in background
        n = len(human_gene_list)  # Genes in query list
        ...
        # Convert Human gene names back to Mouse for display
        mouse_genes_in_set = convert_gene_list(list(genes_in_set), human_to_mouse, 'human_to_mouse')
        
        results.append({
            'Term': gene_set_name,
            'Overlap': f"{x}/{len(human_gene_list)}",
            'P-value': p_value,
            'Adjusted P-value': p_value,  # Will be adjusted later
            'Odds Ratio': odds_ratio,
            'Combined Score': x * np.log10(p_value) if p_value > 0 else 0,
            'Genes': ', '.join(sorted(mouse_genes_in_set))
        })
